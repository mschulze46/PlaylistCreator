// TextFile.cpp: implementation for the textfile class.
//
//////////////////////////////////////////////////////////////////////
    ...
    while(!Eof())
    {
        if(ReadLine(temp))
        {
            ...
            if(buffpos + (int) temp.size() < buffsize)
            {
                ...
                strncpy_s(buffer+buffpos, buffsize-buffpos, temp.c_str(), _TRUNCATE);
#else
...
bool CTextFileRead::Read(wstring& all, const wstring newline)
{
    ...
    while(!Eof())
    {
        if(ReadLine(temp))
        {
            ...
            {
#if _MSC_VER >= 1400  // VS2005
                wcsncpy_s(buffer+buffpos, buffsize-buffpos, temp.c_str(), _TRUNCATE);
#else
...
bool CTextFileRead::Read(CStringArray& all)
{
    all.RemoveAll();

    CString newline = _T("\r\n");
    int lenNewline = newline.GetLength();

    CString temp;
    if(!Read(temp, newline))
        return false;

    int posNewline = 0;
    while(!temp.IsEmpty())
    {
        posNewline = temp.Find(newline);
        if(posNewline < 0) // no newline found
        {
            all.Add(temp);
            temp.Empty();
        }
        ...
        {
            all.Add(temp.Left(posNewline));
            temp.Delete(0, posNewline + lenNewline);
        }
    }
