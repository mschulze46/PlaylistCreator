...
#if _MSC_VER >= 1400  // VS2005
					_tcsncpy_s( drive, _countof(drive), strComputer, _TRUNCATE );
#else
   ...
   ============================================================*/
{
	...
	if( result )
	{
		...
		{
			// the following check is necessary because ::FindFirstFile() fails for
            // folders where access is denied (e.g. "C:\System Volume Information").
            // not setting 'result = false' enables the enumeration process to continue.
            if(::GetLastError() != 5) // 5: Access is denied.
                result = false;
