...
// bro      2002-10-29
//========================================================
void _tcsecpy( std::string* pString, int escape, LPTSTR srt, LPTSTR end = NULL )
{
	*pString = _T("");
	LPTSTR pch = srt;
	if( end==NULL ) end = (LPTSTR)sizeof(long);
		...
		if( xml = _tcsskip( xml ) )
		{
			...
			if( pEnd == NULL ) 
			{
				...
				if( pi->erorr_occur == false ) 
				{
					...
					TCHAR err[1024];
					sprintf(err, _T("<%.768s> attribute has error "), name.c_str() );
					pi->error_string = err;
				}
				return NULL;
			}
			...
			if( XIsEmptyString( value ) )
			{
				...
				if( pEnd == NULL ) 
				{
					if( pi->erorr_occur == false ) 
					{
						...
						TCHAR err[1024];
						sprintf(err, _T("%.256s must be closed with </%.256s>"), name.c_str(), name.c_str() );
						pi->error_string = err;
					}
					...
					if( xml = _tcsskip( xml ) )
					{
						...
						if( pEnd == NULL ) 
						{
							if( pi->erorr_occur == false ) 
							{
								...
								pi->error_code = PIE_NOT_CLOSED;
								TCHAR err[1024];
								sprintf(err, _T("it must be closed with </%.768s>"), name.c_str() );
								pi->error_string = err;
							}
							// error
							return NULL;
						...
						else
						{
							...
							if( pi->force_parse == false )
							{
								...
								if( pi->erorr_occur == false ) 
								{
									...
									TCHAR err[1024];
									sprintf(err, _T("'<%.256s> ... </%.256s>' is not wel-formed."), name.c_str(), closename.c_str());
									pi->error_string = err;
								}
								return NULL;
							}
...
// bro      2002-10-29
//========================================================
std::string _tagXMLNode::GetXML( LPDISP_OPT opt /*= &optDefault*/ ) const
{
	std::ostringstream os;
...
std::string _tagXMLNode::GetText( LPDISP_OPT opt /*= &optDefault*/ ) const
{
	std::ostringstream os;
...
std::string	_tagXMLNode::GetChildText( LPCTSTR name, LPDISP_OPT opt /*= &optDefault*/ )
{
	LPXNode node = GetChild( name );
...
int _tagXMLEntitys::Ref2Entity( LPCTSTR estr, std::string& str, int elen )
{
...
int _tagXMLEntitys::Entity2Ref( LPCTSTR str, std::string& estr, int len )
{
...
std::string _tagXMLEntitys::Ref2Entity( LPCTSTR estr )
{
	std::string es;
	if( estr )
	{
...
std::string _tagXMLEntitys::Entity2Ref( LPCTSTR str )
{
	std::string s;
	if( str )
	{
