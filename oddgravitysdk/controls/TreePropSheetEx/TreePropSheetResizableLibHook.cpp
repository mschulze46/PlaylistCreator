...
void CResizableSheetHook::Initialize()
{
  ...
  // Perform remaining intialization.

  // From OnCreate
	// Create and init the size-grip. We only do this for child windows.
...
{
	// Handle the tree cllbacks in another methods.
  if( m_pTreePropSheet->IsTreeViewMode() )    // Tree mode
  {
...
BOOL CTreePropSheetResizableLibHook::TreeModeCallbacks(LayoutInfo &layout)
{
  ...
  { // Tab control
    ...
    // is resized by 'anchors', it is garanteed to be already resized 
    // properly.
