...
/*! @brief Base class for CTreePropSheetEx
  ...
  See CTreePropSheet for more details on class functioanlity.

  @internal 
    CTreePropSheetBase is an update of CTreePropSheet. 
    Accessiblity of several methods has been chaged as well as 
...
public:
  ...
  //@}
	...
	for empty parent nodes. The list contains exactly two images:
...
public:
  ...
  @internal
    Made virtual in ordeer to support resizing after the control has 
    been created.
	*/
	virtual BOOL SetTreeWidth(int nWidth);

	/**
	Sets the TreeCtrl's font and color

	@param lf
		Specifies the TreeCtrl's font
	@param crTextColor
		Sets the text color
	@param crBackgroundColor
		Sets the background color
	*/
	void SetTreeCtrlFont(LOGFONT& lf, COLORREF crTextColor = NULL, COLORREF crBackgroundColor = NULL);

	/**
	Sets the TreeCtrl's images. Those images can have larger size than
	16x16 pixel. The images used for the PropPage caption keep their size.

	@param images
	Image list for the TreeCtrl items
	@param cyIndent
	Number of pixels added to the top and bottom of an item
	*/
	void SetTreeCtrlImages(CImageList& images, int cyIndent = 0);
...
protected:
	...
	*/
	...
	/**
	Returns the page tree item, that representes the specified page
...
private:
	...
	/** The tree control's font */
	CFont m_fontPageTree;

	/** The tree control's text color */
	...
	CImageList m_Images;		 // images used for the PropPage caption
	CImageList m_ImagesTreeCtrl; // images used for the TreeCtrl items
	
	/** Y-Axis indentation of a TreeCtrl item */
	int m_cyIndentation;
