...
CTreePropSheetBase::~CTreePropSheetBase()
{
	...
	if( m_ImagesTreeCtrl.GetSafeHandle() )
		m_ImagesTreeCtrl.DeleteImageList();
...
void CTreePropSheetBase::SetTreeCtrlFont(LOGFONT& lf, COLORREF crTextColor /*=NULL*/, COLORREF crBackgroundColor /*=NULL*/)
{
	// create font
	m_fontPageTree.CreateFontIndirect(&lf);
...
void CTreePropSheetBase::ActivatePreviousPage(HTREEITEM hItem)
{
    ...
    // we know there is at least one valid property page in the sheet.
		if( m_bSkipEmptyPages && !HasValidPropertyPageAssociated( hPrevItem ) )
...
void CTreePropSheetBase::ActivateNextPage(HTREEITEM hItem)
{
    ...
    // a property page, call ActivateNextPage again. This is fine since
    // we know there is at least one valid property page in the sheet.
		if( m_bSkipEmptyPages && !HasValidPropertyPageAssociated( hNextItem ) )
...
HTREEITEM CTreePropSheetBase::GetNextValidPageTreeItem(HTREEITEM hTreeItem) const
{
	...
	while (hItem)
	{
		if ( HasValidPropertyPageAssociated( hItem ) )
			return hItem;
...
BOOL CTreePropSheetBase::OnInitDialog() 
{
  ...
  if (!m_bTreeViewMode)
	...
	#if _MFC_VER >= 0x0700
	{
		...
		//	WS_EX_CLIENTEDGE|WS_EX_NOPARENTNOTIFY,
		//	WC_TREEVIEW, _T("SysTreeView32"),
		//	WS_TABSTOP|WS_CHILD|WS_VISIBLE|dwTreeStyle,
		//	rectTree.left, rectTree.top, rectTree.Width(), rectTree.Height(),
		//	this->m_hWnd, NULL, ::AfxGetApp()->m_hInstance, NULL);

		//m_pwndPageTree->Attach(hTree);
		//m_pwndPageTree->SetDlgCtrlID(s_unPageTreeId);
	...
	// set the TreeCtrl images
	if (m_bTreeImages)
	{
		if(m_ImagesTreeCtrl.GetSafeHandle())
		{
			m_pwndPageTree->SetImageList(&m_ImagesTreeCtrl, TVSIL_NORMAL);
			m_pwndPageTree->SetImageList(&m_ImagesTreeCtrl, TVSIL_STATE);
		}
		else
		{
	...
	// set indentation for TreeCtrl items
	if(m_cyIndentation > 0)
	{
		IMAGEINFO ii;
		m_ImagesTreeCtrl.GetImageInfo(0, &ii);
		int iHeight = ii.rcImage.bottom - ii.rcImage.top;
		m_pwndPageTree->SetItemHeight(iHeight + (2*m_cyIndentation));
	}

	// set special font if we have one
	if(m_fontPageTree.GetSafeHandle())
	{
		m_pwndPageTree->SetFont(&m_fontPageTree);
		m_pwndPageTree->SetTextColor(m_crTextColor);
		m_pwndPageTree->SetBkColor(m_crBackgroundColor);
	}
...
void CTreePropSheetBase::OnDestroy() 
{
	...
	////////// fix 1 //////////
	//::DestroyWindow(m_pwndPageTree->Detach());
...
void CTreePropSheetBase::SetTreeCtrlImages(CImageList& images, int cyIndent /*=0*/)
{
	// delete old image list if we have one
	if(m_ImagesTreeCtrl.GetSafeHandle())
		m_ImagesTreeCtrl.DeleteImageList();

	// attach image list
	m_ImagesTreeCtrl.Create(&images);

	// set y-axis indentation
	m_cyIndentation = cyIndent;
}
