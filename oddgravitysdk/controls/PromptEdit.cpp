...
void CPromptEdit::SetPromptText(LPCTSTR lpszPrompt)
{
	...
	if(!GetPromptTextActive())
        CEdit::SetWindowText((LPCTSTR)m_strPrompt);
}
...
void CPromptEdit::SetPromptText(LPCTSTR lpszPrompt, bool bRegainPrompt, PromptShowModeEnum PromptShowMode)
{
    ...
    SetRegainPrompt(bRegainPrompt);
    SetPromptShowMode(PromptShowMode);
}
...
{	
    ...
    CEdit::GetWindowText(strWindowTextTrimmed);
	
	// Get rid of spaces and tabs
    strWindowTextTrimmed.Trim();
	
	return strWindowTextTrimmed.GetLength();
}

int CPromptEdit::GetWindowText(LPTSTR lpszStringBuf, int nMaxCount)
{
    if (GetPromptTextActive())
	{
...
void CPromptEdit::GetWindowText(CString& rString)
{
    if (GetPromptTextActive())
		rString = _T("");
	else
...
void CPromptEdit::SetWindowText(LPCTSTR lpszString)
{
...
{	
	if(GetPromptTextActive())
	{
        CEdit::SetWindowText(m_strPrompt);
		return FALSE;
	}		
...
void CPromptEdit::OnRButtonDown(UINT nFlags, CPoint point)
{	
...
{	
    ...
    if(::GetFocus() == m_hWnd || !GetPromptTextActive())
