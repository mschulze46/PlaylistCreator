...
CHyperLink::CHyperLink(LOGFONT lf)
{
	__init__();
...
void CHyperLink::__init__()
{
	ZeroMemory(&m_logfont, sizeof(m_logfont));
	m_hLinkCursor       = NULL;                 // No cursor as yet
    m_bIsDefaultCursor  = FALSE;                // Default cursor not yet loaded
	...
	m_strURL.Empty();
	m_nTimerID          = 100;
	m_bIsToolTipActivated = TRUE;
}

CHyperLink::~CHyperLink()
{
	if (m_fontStandard.m_hObject)
	{
		m_fontStandard.DeleteObject();
	}

	if (m_fontUnderlined.m_hObject)
	{
		m_fontUnderlined.DeleteObject();
	}

    if (m_hLinkCursor != NULL && m_bIsDefaultCursor)
    {
        VERIFY(DestroyCursor(m_hLinkCursor));
    }
}
...
void CHyperLink::PreSubclassWindow() 
{
	...
	CFont* pFont = NULL;
	if(CString(m_logfont.lfFaceName).IsEmpty())
	{
		pFont = GetFont();
		if (!pFont)
		{
...
void CHyperLink::OnClicked()
{
    ...
    UINT result = (UINT)GotoURL(m_strURL, SW_SHOW);
    ...
    if (!m_bOverControl)        // Cursor has just moved over control
    {
        ...
        if(m_strURL.GetLength())
        {
            if (m_nUnderline == ulHover)
			    CStatic::SetFont(&m_fontUnderlined);
        }
        Invalidate();
...
void CHyperLink::OnTimer(UINT nIDEvent) 
{
    ...
    if (!rect.PtInRect(p))
    {
        ...
        if (m_nUnderline != ulAlways)
			CStatic::SetFont(&m_fontStandard);
        rect.bottom+=10;
        InvalidateRect(rect);
    }
...
{
    if (m_hLinkCursor && m_strURL.GetLength())
    {
...
void CHyperLink::SetFont(CFont* pFont, BOOL bRedraw /*=TRUE*/)
{
...
void CHyperLink::UpdateFonts(LOGFONT& lf)
{
	if(m_fontStandard.m_hObject)
	{
		m_fontStandard.DeleteObject();
	...
	SetUnderline(m_nUnderline, TRUE);
}
...
void CHyperLink::SetUnderline(int nUnderline /*=ulHover*/, BOOL bForce /*=FALSE*/)
{
	if (m_nUnderline == nUnderline && !bForce)
        return;
...
void CHyperLink::PositionWindow()
{
    ...
    CDC* pDC = GetDC();
    CFont* pOldFont = pDC->SelectObject(&m_fontUnderlined);
...
void CHyperLink::ActivateToolTip(BOOL bActivate)
{
	m_bIsToolTipActivated = bActivate;
	m_ToolTip.Activate(m_bIsToolTipActivated);
}
