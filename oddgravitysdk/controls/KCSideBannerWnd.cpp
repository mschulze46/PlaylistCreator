...
, m_uTextureAlignment(KCSB_TEXTURE_LEFT)
...
void CKCSideBannerWnd::DrawBackground(CDC* pDC, CRect rect)
{
	...
	else if ( m_uFillFlag & KCSB_FILL_TEXTURE )
	{
		...
		if((m_uTextureAlignment == KCSB_TEXTURE_RIGHT) && (rect.Width() > bmpInfo.bmWidth))
		{
			pDC->FillSolidRect(&rect, m_colBkg);

			CRect rectTexture = rect;
			rectTexture.left = rect.right - bmpInfo.bmWidth;

			CDC memDC;
			memDC.CreateCompatibleDC(pDC);
			CBitmap bmpComp;
			bmpComp.CreateCompatibleBitmap(pDC, rectTexture.Width(), rectTexture.Height());
			CBitmap* pOldBmp = (CBitmap*)memDC.SelectObject(&bmpComp);
			memDC.SelectObject(bmp);
			pDC->BitBlt(rectTexture.left, rectTexture.top, rectTexture.Width(), rectTexture.Height(), &memDC, 0, 0, SRCCOPY);
			memDC.SelectObject(pOldBmp);
		}
		else
		{
...
void CKCSideBannerWnd::SetTexture(HBITMAP hBitmap, bool bSelfDelete, unsigned int uAlignment)
{
	if ( m_hBkgBitmap && m_bBmpDelete )
		::DeleteObject(m_hBkgBitmap);
	m_bBmpDelete = bSelfDelete;
	m_hBkgBitmap = hBitmap;
	m_uTextureAlignment = uAlignment;
}
