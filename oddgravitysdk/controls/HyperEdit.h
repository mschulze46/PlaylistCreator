...
public:
    ...
    void EnableFontSmoothingFix(COLORREF clrBackground) { m_clrBackground = clrBackground; m_bFontSmoothingFixEnabled = true; }
...
private:
    ...
    CFont m_fontOverlay;        // font used for overwriting URLs in the original text (same font as the control but with underline style)

    CString m_csPreviousUrl;    // URL of file processed in the previous run
    ...
    bool m_bFontSmoothingFixEnabled;    // if font smoothing fix is enabled the background of links is filled
    COLORREF m_clrBackground;           // with the specified background color before painting the link text
