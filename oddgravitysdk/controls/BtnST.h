...
// =================================
//
// Version 1.0: fixed flickering, fixed disappearing tooltips, added tooltip color support
// Version 1.1: added support for separate icons for all button states, added 'no offset' pressed style
// Version 1.2: fixed redraw bug when enabling/disabling a button, added font support
// Version 1.3: fixed typeless usage of enums, improved drawing of focus rect and accelerator
// Version 1.4: added overload method for setting the cursor
//
//////////////////////////////////////////////////////////////////////////////////////////////////////////
...
public:
    ...
    enum ButtonAlignmentStyle
    {
        BTNST_ALIGN_HORZ = 0,           // Image on the left, text on the right
    ...
    enum ButtonPressedStyle
    {
        BTNST_PRESSED_LEFTRIGHT = 0,    // Pressed from left to right (usual behavior)
    ...
    enum ButtonColor
    {
        ...
        BTNST_MAX_COLORS                // must remain the last item
    };
...
public:
	LOGFONT* GetFont();
	BOOL SetFont(LOGFONT* pFont);
	BOOL SetFont(CString sFontName, long lSize=0, long lWeight=400, BYTE bItalic=0, BYTE bUnderline=0);

	DWORD SetDefaultColors(BOOL bRepaint = TRUE);
	DWORD SetColor(ButtonColor btnColor, COLORREF crColor, BOOL bRepaint = TRUE);
	DWORD GetColor(ButtonColor btnColor, COLORREF* crpColor);
	DWORD OffsetColor(ButtonColor btnColor, short shOffset, BOOL bRepaint = TRUE);

	DWORD SetBackgroundColorForAllStates(COLORREF crColor, BOOL bRepaint = TRUE);
	DWORD SetTextColorForAllStates(COLORREF crColor, BOOL bRepaint = TRUE);
	...
	void SetTooltipColor(COLORREF crText, COLORREF crBackground);
	...
	DWORD SetAlignmentStyle(ButtonAlignmentStyle eAlignment, BOOL bRepaint = TRUE);
	DWORD SetPressedStyle(ButtonPressedStyle ePressedStyle, BOOL bRepaint = TRUE);
	...
	void SetIconOffsetX(int iOffsetX);
	void SetIconOffsetY(int iOffsetY);
	void SetIconOffset(int iOffsetX, int iOffsetY);

	void SetTextOffsetX(int iOffsetX);
	void SetTextOffsetY(int iOffsetY);
	void SetTextOffset(int iOffsetX, int iOffsetY);
...
protected:
    ...
    ButtonAlignmentStyle m_eAlignmentStyle; // Button alignment style
	BOOL		m_bDrawBorder;		// Draw border?
	BOOL		m_bDrawFlatFocus;	// Draw focus rectangle for flat button?
	COLORREF	m_crColors[BTNST_MAX_COLORS];	// Button colors to be used
...
private:
	...
	CString		m_strTooltipText;	// Tooltip's text
	...
	// is only applied when showing text AND icon!
	int			m_iIconOffsetX;		// Icon x-offset from button border
	int			m_iIconOffsetY;		// Icon y-offset...
	int			m_iTextOffsetX;		// Text x-offset from button border
	int			m_iTextOffsetY;		// Text y-offset...
...
#pragma pack()
	...
	STRUCT_ICONS	m_csIcons[4];	// Index: 0=In, 1=Out, 2=Pressed, 3=Disabled
	STRUCT_BITMAPS	m_csBitmaps[4]; // Index: 0=In, 1=Out, 2=Pressed, 3=Disabled
